...
제공된 코드 컨텍스트에는 다음 메타데이터가 포함되어 있습니다:
...
4. 수정이 다른 파일에 영향을 미칠 수 있는지 확인하세요.
...
이 질문에 답변할 때 다음을 고려하세요:
1. 청크 타입(class, method, function, code)에 따라 코드의 목적과 역할을 정확히 파악하세요.
2. 부모-자식 관계와 상속 관계를 고려하여 코드의 구조적 맥락을 유지하세요.
3. 복잡도 점수가 높은 청크는 중요한 비즈니스 로직일 가능성이 높으니 특별히 주의하세요.
4. 역할 태그를 활용하여 코드의 기능적 의도를 파악하세요.
5. 지나치게 긴 답변은 피하고, 필요시 코드 예시를 포함하여 설명하세요.
