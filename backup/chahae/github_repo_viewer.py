...
def get_repo_content_as_document(owner: str, repo: str, path: str, token: str) -> Optional[Document]:
    ...
    try:
        ...
        # 파일 확장자 확인
        file_name = content_data['name'].lower()
        binary_extensions = {'.png', '.jpg', '.jpeg', '.gif', '.bmp', '.ico', '.pdf', 
                           '.exe', '.dll', '.so', '.dylib', '.bin', '.zip', '.tar', 
                           '.gz', '.rar', '.7z', '.pyc', '.pyo', '.pyd'}
        
        # 바이너리 파일인 경우 건너뛰기
        if any(file_name.endswith(ext) for ext in binary_extensions):
            print(f"[건너뛰기] 바이너리 파일: {path}")
            return None
...
def main(token: Optional[str] = None):
    ...
    # 저장소 URL 입력 받기
    while True:
        repo_url = "https://github.com/nmmm9/telco-customer-churn"
